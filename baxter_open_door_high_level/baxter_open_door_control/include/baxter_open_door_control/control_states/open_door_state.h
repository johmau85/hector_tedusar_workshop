...
#ifndef open_door_state_h___
#define open_door_state_h___
...
class OpenDoorState : public BaxterOpenDoorControlState
{
public:
    OpenDoorState(BaxterOpenDoorControl* baxter_open_door_control);

    virtual ~OpenDoorState();

    virtual std::string getStateName()
    {
        return std::string("Open Door State");
    }
