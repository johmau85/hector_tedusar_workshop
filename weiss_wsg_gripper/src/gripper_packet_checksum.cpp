...
const static unsigned short CRC_TABLE[256] = { 0x0000, 0x1021, 0x2042, 0x3063,
...
GripperPacketChecksum::GripperPacketChecksum() {
	reset();
}

void GripperPacketChecksum::reset() {
	crc_ = 0xFFFF;
}

void GripperPacketChecksum::feed(unsigned char data) {
	crc_ = CRC_TABLE[(crc_ ^ data) & 0x00FF] ^ (crc_ >> 8);
}

void GripperPacketChecksum::feed(const std::vector<unsigned char> &data) {
	std::vector<unsigned char>::const_iterator it;
	for (it = data.begin(); it != data.end(); ++it)
		feed(*it);
}

unsigned short GripperPacketChecksum::getValue() {
	return crc_;
}
